export const ownerActionSelect = `
  ...
  owner_action_attachments(id,file_name,mime_type,byte_size,width,height,finalized_at),
  owner_action_tasks(task_id,tasks(id,title,status))
`;

export const MAX_OWNER_ACTION_ATTACHMENTS = 10;

export function onlyFinalizedOwnerActionAttachments<
  T extends {
    owner_action_attachments: Array<{ finalized_at: string | null }>;
  },
>(action: T) {
  return {
    ...action,
    owner_action_attachments: action.owner_action_attachments.filter(
      (attachment) => attachment.finalized_at !== null,
    ),
  };
}
