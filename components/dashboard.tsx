...
export function Dashboard({
  ...
  async function createOwnerAction(input: {
    title: string;
    notes?: string;
    dueAt?: string | null;
    taskId?: string;
  }) {
    const body = await requestJson("/api/owner-actions", {
      method: "POST",
    ...
    if (input.taskId) {
      const linked = await requestJson(
        `/api/owner-actions/${action.id}/tasks`,
        { method: "POST", body: JSON.stringify({ taskId: input.taskId }) },
      );
  ...
  async function updateOwnerAction(
    actionId: string,
    updates: Record<string, unknown>,
  ) {
    const body = await requestJson(`/api/owner-actions/${actionId}`, {
      method: "PATCH",
  ...
  async function linkOwnerAction(actionId: string, taskId: string) {
    const body = await requestJson(`/api/owner-actions/${actionId}/tasks`, {
      method: "POST",
  ...
  async function unlinkOwnerAction(actionId: string, taskId: string) {
    await requestJson(`/api/owner-actions/${actionId}/tasks/${taskId}`, {
      method: "DELETE",
    });
  ...
  async function saveOwnerAction(event: FormEvent) {
    ...
    if (!selectedOwnerAction) return;
    await runAction(async () => {
      await updateOwnerAction(selectedOwnerAction.id, {
        title: ownerActionTitle,
        notes: ownerActionNotes,
        dueAt: ownerActionDueAt ? localDateTimeToUtc(ownerActionDueAt) : null,
      });
      setEditingOwnerAction(false);
    });
  }
  ...
  async function updateTask(event: FormEvent) {
    ...
    await runAction(async () => {
      ...
      if (editImage) {
        ...
        if (existing)
          await requestJson(
            `/api/tasks/${editingTask.id}/attachments/${existing.id}`,
            { method: "DELETE" },
          );
        body.task.task_attachments = [
          await uploadAttachment(editingTask.id, editImage),
        ];
      }
      setTasks((current) =>
        current.map((task) => (task.id === body.task.id ? body.task : task)),
      );
      setEditingTask(null);
      setEditTitle("");
      setEditInstructions("");
    ...
    <main className="app-shell">
      ...
      {area === "my-work" ? (
        <MyWork
          ...
          busy={busy}
          runAction={runAction}
          onCreate={createOwnerAction}
          onUpdate={updateOwnerAction}
        ...
        <section className="workspace">
          <aside className={`task-column ${selected ? "has-selection" : ""}`}>
            <div className="task-column-heading">
              <div>
                <p className="eyebrow">Your relay</p>
                <h1>Tasks in motion</h1>
              </div>
              <button
                ...
                <div className="empty-state">
                  ...
                  <p>
                    {tasks.length
                      ...
                      : "Write the outcome and context. Your laptop can take it from there."}
                  </p>
                  {!tasks.length && (
                    <button
          ...
          <section className={`detail-panel ${selected ? "open" : ""}`}>
            {selected ? (
              <TaskDetail
                ...
                onCreateOwnerAction={(title) =>
                  runAction(() =>
                    createOwnerAction({ title, taskId: selected.id }),
                  )
                }
                onUpdateOwnerAction={(actionId, updates) =>
                  runAction(() => updateOwnerAction(actionId, updates))
                }
                onLinkOwnerAction={(actionId) =>
                  runAction(() => linkOwnerAction(actionId, selected.id))
                }
                onUnlinkOwnerAction={(actionId) =>
                  runAction(() => unlinkOwnerAction(actionId, selected.id))
                }
              ...
              <div className="detail-placeholder">
                <CircleDot size={30} />
                <h2>Select a task</h2>
                <p>Its durable context and run results will appear here.</p>
              </div>
            )}
          </section>
        </section>
          ...
          >
            ...
            <form onSubmit={createTask}>
              ...
              <div className="composer-hint">
                ...
                <span>
                  {draft.instructions.length.toLocaleString()} characters
                </span>
              </div>
      ...
      {selectedOwnerAction && (
        ...
        >
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="owner-action-detail-title"
            onMouseDown={(event) => event.stopPropagation()}
          >
            <div className="sheet-heading">
              <div>
                <p className="eyebrow">Owner action</p>
                <h2 id="owner-action-detail-title">
                  {editingOwnerAction
                    ? "Edit action"
                    : selectedOwnerAction.title}
                </h2>
              </div>
              <button
                className="icon-button"
            ...
            {editingOwnerAction ? (
              <form onSubmit={saveOwnerAction}>
                <label htmlFor="detail-owner-action-title">Action title</label>
                <input
                  id="detail-owner-action-title"
                  value={ownerActionTitle}
                  onChange={(event) => setOwnerActionTitle(event.target.value)}
                  required
                  maxLength={160}
                  autoFocus
                />
                <label htmlFor="detail-owner-action-notes">
                  Notes (optional)
                </label>
                <textarea
                  id="detail-owner-action-notes"
                  value={ownerActionNotes}
                  onChange={(event) => setOwnerActionNotes(event.target.value)}
                  maxLength={20000}
                  rows={5}
                />
                <label htmlFor="detail-owner-action-due">
                  Due date (optional)
                </label>
                <input
                  id="detail-owner-action-due"
                  type="datetime-local"
                  value={ownerActionDueAt}
                  onChange={(event) => setOwnerActionDueAt(event.target.value)}
                />
                <div className="edit-actions">
                  <button
                    type="button"
                    className="secondary-button"
                    onClick={() => setEditingOwnerAction(false)}
                  >
                    Cancel
                  </button>
                  <button className="primary-button" disabled={busy}>
              ...
              <div className="owner-action-detail">
                <div className="action-meta">
                  <span
                    className={`owner-status ${selectedOwnerAction.status}`}
                  >
                    {ownerActionStatusLabel(selectedOwnerAction)}
                  </span>
                  {selectedOwnerAction.due_at && (
                    <span>
                      <CalendarDays size={13} />
                      Due {formatDate(selectedOwnerAction.due_at)}
                    </span>
                  )}
                </div>
                <section>
                ...
                <section>
                  ...
                  {selectedOwnerAction.owner_action_tasks.length ? (
                    <div className="owner-action-links">
                      {selectedOwnerAction.owner_action_tasks.map(
                        (link) =>
                          link.tasks && (
                            <button
                              key={link.task_id}
                              onClick={() => openTask(link.task_id)}
                            >
                              {link.tasks.title}
                ...
                <div className="edit-actions">
                  <button
                    ...
                    disabled={busy}
                    onClick={() =>
                      startTaskFromOwnerAction(selectedOwnerAction)
                    }
                  >
                    <Plus size={17} />
                    Task
                  ...
                  <button
                    className="primary-button"
                    disabled={busy}
                    onClick={startEditingOwnerAction}
                  >
                    <Pencil size={17} />
                    Edit action
                  </button>
      ...
      {workerOpen && (
        <div
          className="modal-backdrop"
          onMouseDown={() => setWorkerOpen(false)}
        >
          <section
            className="sheet worker-sheet"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="sheet-heading">
              <div>
                <p className="eyebrow">Outbound only</p>
                <h2 id="worker-title">Worker setup</h2>
              </div>
              <button
                className="icon-button"
            ...
            {workers.length > 0 && (
              <div className="worker-list">
                {workers.map((worker) => (
                  <div key={worker.id}>
                    <span
                      className={`worker-dot ${worker.last_seen_at ? "seen" : ""}`}
                    />
                    <div>
                      <strong>{worker.name}</strong>
                      <p>
                        {worker.last_seen_at
                          ? `Seen ${relativeDate(worker.last_seen_at)}`
                          : "Not connected yet"}
                      </p>
                    </div>
                  </div>
            ...
            {workerToken ? (
              <div className="token-panel">
                <p>Copy this token now. Relay stores only its hash.</p>
                <code>{workerToken}</code>
                <button
                  className="secondary-button"
              ...
              <form onSubmit={createWorker}>
                <label htmlFor="worker-name">Laptop name</label>
                <input
                  id="worker-name"
                  name="name"
...
function MyWork({
  ...
  const visible = useMemo(() => {
    ...
    return actions
      .filter((action) => ownerActionFilter(action) === filter)
      .filter(
        (action) =>
          !normalized ||
          [
            action.title,
            action.notes,
            ...action.owner_action_tasks.map((link) => link.tasks?.title ?? ""),
  ...
  async function create(event: FormEvent) {
    ...
    await runAction(async () => {
      await onCreate({
        title,
        notes,
        dueAt: dueAt ? localDateTimeToUtc(dueAt) : null,
      });
      setTitle("");
      setNotes("");
      setDueAt("");
  ...
  function startHiding(action: OwnerAction) {
    ...
    const initial =
      action.due_at && new Date(action.due_at) < tomorrow
        ? action.due_at
  ...
  function quickHide(days: number) {
    setSnoozeUntil(
      utcToLocalDateTime(
        new Date(hideStartedAt + days * 86400000).toISOString(),
      ),
    );
  }
  ...
  return (
    <section className="my-work-view">
      ...
      <div className="filter-strip" aria-label="Filter owner actions">
        {(["active", "snoozed", "done"] as const).map((item) => (
          ...
          >
            ...
            <span>
              {
                actions.filter((action) => ownerActionFilter(action) === item)
                  .length
              }
            </span>
          </button>
      ...
      <label className="owner-action-search">
        <Search size={17} />
          ...
          visible.map((action) => (
            <article className="owner-action-card" key={action.id}>
              ...
              <div className="owner-action-copy">
                ...
                <div className="action-meta">
                  <span className={`owner-status ${action.status}`}>
                    {ownerActionStatusLabel(action)}
                  </span>
                  {filter === "snoozed" && action.snoozed_until && (
                    <span>
                      <Clock3 size={13} />
                  ...
                  {action.due_at && (
                    <span
                      className={
                        new Date(action.due_at) <= new Date() &&
                        action.status !== "done"
                          ? "overdue"
                          : ""
                      }
                  ...
                  <span>
                    {action.owner_action_tasks.length} linked{" "}
                    {action.owner_action_tasks.length === 1 ? "task" : "tasks"}
                  </span>
                </div>
                {action.owner_action_tasks.length > 0 && (
                  <div className="linked-task-titles">
                    {action.owner_action_tasks.map(
                      (link) =>
                        link.tasks && (
                          <button
                            key={link.task_id}
              ...
              <div className="action-controls">
                  ...
                  <button
                    onClick={() =>
                      runAction(() =>
                        onUpdate(action.id, {
                          status:
                            action.status === "todo" ? "in_progress" : "todo",
                        }),
                      )
                    }
                  >
                    {action.status === "todo" ? "Start" : "To do"}
                  </button>
                )}
                {filter === "snoozed" ? (
                  <button
                    onClick={() =>
                      runAction(() =>
                        onUpdate(action.id, { snoozedUntil: null }),
                      )
                    }
                  >
                    Show now
                  ...
                  action.status !== "done" && (
                    <button
                      disabled={Boolean(
                        action.due_at && new Date(action.due_at) <= new Date(),
                      )}
                      title={
                        action.due_at && new Date(action.due_at) <= new Date()
                          ? "This action is already due and must stay visible."
                          : undefined
                      }
                      onClick={() => startHiding(action)}
                    >
                      Hide until
                    </button>
                  )
                ...
                {filter !== "snoozed" &&
                  ...
                  action.due_at &&
                  new Date(action.due_at) <= new Date() && (
                    <span className="hide-disabled-note">
                      Already due — cannot be hidden.
                    </span>
                  )}
              </div>
            </article>
          ...
          >
            <div className="sheet-heading">
              <div>
                ...
                <h2 id="new-action-title">New action</h2>
              </div>
              <button
                className="icon-button"
                onClick={() => setComposerOpen(false)}
            ...
            <form onSubmit={create}>
              <label htmlFor="owner-action-title">Action title</label>
              <input
                id="owner-action-title"
                value={title}
                ...
                autoFocus
              />
              <label htmlFor="owner-action-notes">Notes (optional)</label>
              <textarea
                id="owner-action-notes"
                value={notes}
                onChange={(event) => setNotes(event.target.value)}
                maxLength={20000}
                rows={5}
              />
              <label htmlFor="owner-action-due">Due date (optional)</label>
              <input
                id="owner-action-due"
                type="datetime-local"
                value={dueAt}
      ...
      {snoozing && (
        <div className="modal-backdrop" onMouseDown={() => setSnoozing(null)}>
          <section
            className="sheet small-sheet"
            role="dialog"
            aria-modal="true"
            aria-labelledby="hide-action-title"
            onMouseDown={(event) => event.stopPropagation()}
          >
            <div className="sheet-heading">
              <div>
                <p className="eyebrow">Remind me on</p>
                <h2 id="hide-action-title">Hide {snoozing.title}</h2>
              </div>
              <button className="icon-button" onClick={() => setSnoozing(null)}>
                <X size={20} />
              </button>
            </div>
            <form
              onSubmit={(event) => {
                ...
                void runAction(async () => {
                  await onUpdate(snoozing.id, {
                    snoozedUntil: localDateTimeToUtc(snoozeUntil),
                  });
                  setSnoozing(null);
                });
              }}
            >
              <p className="sheet-intro">
                The due date will not change. This action returns to Active at
                the selected time or when it is due, whichever comes first.
              </p>
              <div className="hide-quick-choices">
                <button
                  type="button"
                  disabled={Boolean(
                    snoozing.due_at &&
                      new Date(hideStartedAt + 86400000) >
                        new Date(snoozing.due_at),
                  )}
                  onClick={() => quickHide(1)}
                >
                  Tomorrow
                ...
                <button
                  ...
                  disabled={Boolean(
                    snoozing.due_at &&
                      new Date(hideStartedAt + 7 * 86400000) >
                        new Date(snoozing.due_at),
                  )}
                  onClick={() => quickHide(7)}
                >
              ...
              <label htmlFor="hide-until">Pick date and time</label>
              <input
                id="hide-until"
                type="datetime-local"
                min={nextLocalDateTimeMinute(new Date(hideStartedAt))}
                max={utcToLocalDateTime(snoozing.due_at)}
                value={snoozeUntil}
...
function TaskDetail({
  ...
  const failed = [...task.runs]
    .sort((a, b) => b.attempt - a.attempt)
    .find((run) => run.status === "failed");
  const linkedActions = ownerActions.filter((action) =>
    action.owner_action_tasks.some((link) => link.task_id === task.id),
  );
  const availableActions = ownerActions.filter(
    (action) => action.status !== "done" && !linkedActions.includes(action),
  );
  ...
  return (
    <div className="detail-content">
      ...
      {task.task_attachments.map((attachment) => (
        <section className="attachment-section" key={attachment.id}>
          ...
          >
            ...
            <img
              src={`/api/tasks/${task.id}/attachments/${attachment.id}`}
      ...
      <section className="your-actions-section">
        <div className="section-label">
          <span>Your actions</span>
          <button
            className="document-edit-button"
            disabled={busy}
            onClick={() => onCreateOwnerAction(`Action for ${task.title}`)}
          >
            <Plus size={14} />
            Create linked
          </button>
        ...
        {linkedActions.length ? (
          <div className="task-actions-list">
            {linkedActions.map((action) => (
              <div key={action.id}>
                <div>
                  <button
                    className="linked-action-title"
                    onClick={() => onOpenOwnerAction(action.id)}
                  >
                    {action.title}
                  </button>
                  <span>{ownerActionStatusLabel(action)}</span>
                </div>
                <div>
                  <button
                    onClick={() =>
                      onUpdateOwnerAction(action.id, {
                        status: action.status === "done" ? "todo" : "done",
                      })
                    }
                  >
                  ...
                  <button onClick={() => onUnlinkOwnerAction(action.id)}>
                    Unlink
                  </button>
                </div>
              </div>
          ...
          <p className="no-actions">
            No owner actions are linked to this task.
          </p>
        )}
        {availableActions.length > 0 && (
          <label className="link-existing">
            Link existing action
            <select
              defaultValue=""
              onChange={(event) => {
                if (event.target.value) onLinkOwnerAction(event.target.value);
                event.target.value = "";
              }}
            >
      ...
      {active && (
        <section className="run-status-card">
          ...
          <div>
            ...
            <p>
              {active.status === "queued"
                ? "This run stays safely queued while your worker is offline."
                : "The result will appear here when the worker finishes."}
            </p>
          </div>
        </section>
      )}

      {failed && !active && !latest && (
        <section className="error-card">
          <p className="eyebrow">Attempt {failed.attempt} failed</p>
          <h2>The worker could not finish this run.</h2>
          <p>{failed.error}</p>
          <div className="handoff-actions">
            ...
            <button
              ...
              disabled={busy}
              onClick={() =>
                onCreateOwnerAction(`Decide whether to retry ${task.title}`)
              }
            >
      ...
      {latest && task.status !== "done" && (
        <section className="result-section">
          <div className="section-label">
            <span>Result · attempt {latest.attempt}</span>
            <span>
              {latest.finished_at
          ...
          <div className="result-handoff">
            <button
              ...
              disabled={busy}
              onClick={() => onCreateOwnerAction(`Review ${task.title}`)}
            >
              <UserRoundCheck size={17} />
              Add to My Work
            </button>
          </div>
          {latest.result_artifacts.length > 0 && (
            <div className="artifact-list">
              {latest.result_artifacts.map((artifact, index) => (
                <a
          ...
          <div className="review-actions">
            ...
            <textarea
              ...
              placeholder="What should change or be explored next?"
              rows={4}
            />
            <div>
              <button
      ...
      {task.status === "done" && task.accepted_result && (
        <section className="result-section accepted">
          <div className="accepted-heading">
            <div>
              <Check size={18} />
              <span>Accepted result</span>
            </div>
            <button className="secondary-button compact" onClick={onFollowUp}>
              <Plus size={17} />
              Follow-up
            </button>
          </div>
  ...
  return (
    <div className="image-picker">
      <label htmlFor="task-image">
        <Paperclip size={15} />
        {currentName ? "Replace image" : "Attach image"}
      </label>
      <input
        id="task-image"
        type="file"
      ...
      {(file || currentName) && (
        <div className="image-preview">
          ...
          <div>
            <strong>{file?.name ?? currentName}</strong>
...
async function requestJson(path: string, init?: RequestInit) {
  ...
  if (!response.ok)
    throw new Error(body.error || "Relay could not complete that action.");
  return body;
}
