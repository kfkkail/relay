...
export function validateAttachmentRequest(
  ...
  const safeName =
    typeof fileName === "string" ? fileName.trim().slice(0, 255) : "";
...
export async function normalizeImage(
  input: Buffer,
  declaredType: AttachmentMimeType,
) {
  if (input.byteLength > MAX_ATTACHMENT_BYTES)
    throw new ApiError("Image is larger than 10 MB.");
  let metadata;
  try {
  ...
  if (!detectedType || detectedType !== declaredType)
    throw new ApiError("The image contents do not match its declared type.");
  if (
    !metadata.width ||
    !metadata.height ||
    metadata.width > MAX_ATTACHMENT_DIMENSION ||
    metadata.height > MAX_ATTACHMENT_DIMENSION ||
    metadata.width * metadata.height > MAX_ATTACHMENT_PIXELS
  ) {
  ...
  if (data.byteLength > MAX_ATTACHMENT_BYTES)
    throw new ApiError("Normalized image is larger than 10 MB.");
  const normalized = await sharp(data).metadata();
  return {
    data,
    mimeType: detectedType as AttachmentMimeType,
